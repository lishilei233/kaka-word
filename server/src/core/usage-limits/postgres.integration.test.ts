...
test("PostgreSQL limits are atomic, persistent, and clean stale buckets", {
  ...
  const minuteDecisions = await Promise.all(
    Array.from({ length: 11 }, () => limiter.consumeMinute("203.0.113.20")),
  );
  ...
  const storedBuckets = await setupPool.query<{ subject_hash: string }>(
    "SELECT subject_hash FROM picture_word_rate_limit_buckets WHERE scope = 'analyze-minute'",
  );
  ...
  const staleBuckets = await setupPool.query<{ count: number }>(
    "SELECT COUNT(*)::integer AS count FROM picture_word_rate_limit_buckets WHERE scope = 'stale'",
  );
  ...
  await setupPool.query(
    `DELETE FROM picture_word_daily_usage
