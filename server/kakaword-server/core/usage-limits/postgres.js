...
export class PostgresAnalyzeUsageLimiter {
    config;
    logger;
    pool;
    nextCleanupAt = 0;
    ...
    async consumeMinute(clientIP) {
        if (isIP(clientIP) === 0)
            throw new Error("Client IP is unavailable or invalid");
    ...
    async cleanupStaleBucketsIfNeeded() {
        const now = Date.now();
        if (now < this.nextCleanupAt)
            return;
        this.nextCleanupAt = now + CLEANUP_INTERVAL_MS;
        try {
            const result = await this.pool.query("DELETE FROM picture_word_rate_limit_buckets WHERE updated_at < CURRENT_TIMESTAMP - INTERVAL '24 hours'");
