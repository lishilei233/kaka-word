...
export async function analyzeScene(bytes: Uint8Array, maxObjects: number, context: string, signal?: AbortSignal, transport: typeof fetch = fetch) {
...
import { photoCaptionSchema, socialCopySchema, type CaptionReviewInput, type SocialCopyInput } from '../../../server/src/core/image-analysis/types.ts';
...
export async function reviewCaption(bytes: Uint8Array, input: Omit<CaptionReviewInput, 'image' | 'mimeType' | 'signal'>, signal?: AbortSignal, transport: typeof fetch = fetch) {
    ...
    const form = new FormData();
    form.set('image', new Blob([new Uint8Array(bytes)], { type: 'image/jpeg' }), 'photo.jpg');
    form.set('caption', input.caption); form.set('captionChinese', input.captionChinese);
    form.set('words', JSON.stringify(input.words)); form.set('context', input.context ?? '');
    let response: Response;
    try {
        response = await transport(`${baseURL}/v1/studio/caption-review`, { method: 'POST', headers, body: form, signal: signal ? AbortSignal.any([signal, AbortSignal.timeout(120000)]) : AbortSignal.timeout(120000) });
    } catch {
        throw new Error('无法连接现有 AI 服务，请检查 server 是否运行');
    }
    const result = await response.json().catch(() => ({})) as Record<string, unknown>;
    if (!response.ok) throw new Error(typeof result.message === 'string' ? result.message : '照片描述审校失败，原内容未改变');
...
export async function regenerateCaption(bytes: Uint8Array, input: Pick<CaptionReviewInput, 'words' | 'context'>, signal?: AbortSignal, transport: typeof fetch = fetch) {
    ...
    let response: Response;
    try {
        response = await transport(`${baseURL}/v1/studio/caption-regenerate`, { method: 'POST', headers, body: form, signal: signal ? AbortSignal.any([signal, AbortSignal.timeout(120000)]) : AbortSignal.timeout(120000) });
    } catch {
        throw new Error('无法连接现有 AI 服务，请检查 server 是否运行');
    }
    const result = await response.json().catch(() => ({})) as Record<string, unknown>;
