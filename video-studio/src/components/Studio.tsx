...
export function Studio() {
    ...
    async function importFile(file?: File) {
        ...
        await run('导入素材', async () => {
            if (file.type.startsWith('video/')) {
                ...
                update({ ...project, image: undefined, video: undefined, words: [], interaction: undefined, sceneTheme: undefined, caption: '', captionChinese: '', captionReviewRequired: true, captionAudio: undefined, captionAudioSeconds: undefined }); setSelected('');
                ...
                if (imageExt === 'heic' || imageExt === 'heif') {
                    ...
                    setSource('');
                    update({ ...project, image: converted.url, imageWidth: converted.imageWidth, imageHeight: converted.imageHeight, video: undefined, words: [], interaction: undefined, sceneTheme: undefined, caption: '', captionChinese: '', captionReviewRequired: true, captionAudio: undefined, captionAudioSeconds: undefined }); setSelected('');
                    ...
                    try {
                        ...
                        update({ ...project, ...frame, video: undefined, words: [], interaction: undefined, sceneTheme: undefined, caption: '', captionChinese: '', captionReviewRequired: true, captionAudio: undefined, captionAudioSeconds: undefined }); setSelected('');
    ...
    async function importFiles(files: File[]) {
        ...
        await run('导入 Apple 动态照片', async () => {
            ...
            setSource(videoUrl);
            update({ ...project, ...frame, video: videoUrl, captureSeconds: 0, introSeconds: 2, words: [], interaction: undefined, sceneTheme: undefined, caption: '', captionChinese: '', captionReviewRequired: true, captionAudio: undefined, captionAudioSeconds: undefined });
    ...
    async function capture() {
        ...
        await run('截取照片', async () => {
            ...
            update({ ...project, ...frame, video: source, captureSeconds: element.currentTime, words: [], interaction: undefined, sceneTheme: undefined, caption: '', captionChinese: '', captionReviewRequired: true, captionAudio: undefined, captionAudioSeconds: undefined });
    ...
    async function analyze() {
        await run(project.analysisMode === 'scene' ? '分析场景' : '识别物体', async () => {
            ...
            if (project.analysisMode === 'scene') {
                ...
                update({ ...project, words, sceneTheme: result.theme, caption: result.caption, captionChinese: result.captionChinese, captionReviewRequired: false,
            ...
            update({ ...project, words, sceneTheme: undefined, interaction: undefined, cover: undefined, caption: result.caption, captionChinese: result.captionChinese, captionReviewRequired: false, captionAudio: undefined, captionAudioSeconds: undefined, socialCopy: undefined }); setSelected(words[0]?.id || '');
    ...
    async function regenerateCaption() {
        await run('重新生成照片描述', async () => {
            const version = revision.current;
            const reviewed = await api<{ caption: string; captionChinese: string }>('caption', {
            ...
            if (version !== revision.current) return;
            update({ ...project, ...reviewed, captionReviewRequired: false, captionAudio: undefined, captionAudioSeconds: undefined, socialCopy: undefined });
            setMessage('照片描述已根据原图重新生成，词表和互动句未改变');
        });
    }
    async function reviewCurrentCaption() {
        await run('审校当前照片描述', async () => {
            const version = revision.current;
            const reviewed = await api<{ caption: string; captionChinese: string }>('caption-review', {
                image: project.image,
                caption: project.caption,
                captionChinese: project.captionChinese,
                context: project.sceneContext ?? '',
                words: project.words.map(({ english, chinese, kind }) => ({ english, chinese, kind })),
            });
            if (version !== revision.current) return;
            update({ ...project, ...reviewed, captionReviewRequired: false, captionAudio: undefined, captionAudioSeconds: undefined, socialCopy: undefined });
            setMessage('当前照片描述已对照原图审校，可以生成配音');
        });
    }
    async function regenerateInteraction() {
        await run('重新生成互动句', async () => {
            const version = revision.current;
            const result = await api<StudioScene>('scene', { image: project.image, maxObjects: count, context: project.sceneContext ?? '' });
            if (version !== revision.current) return;
            update({ ...project, interaction: { ...project.interaction, ...result.interaction, enabled: project.interaction?.enabled ?? false, audio: undefined, audioSeconds: undefined } });
            setMessage('互动句已根据原图重新生成，照片描述和词表未改变');
        });
    }
    async function speech() {
        if (project.captionReviewRequired) {
            setError('照片描述尚未通过看图审校，请先重新生成照片描述');
            return;
        }
        ...
        <main className="workspace">
            <aside className="panel materials"><div className="panel-heading"><span className="section-number">01</span><h2>素材与单词</h2><ImagePlus size={17} /></div>
                <fieldset disabled={!!busy || !!job || !ready} className="panel-body">
                    ...
                    <SceneEditor project={project} update={update} onEditArrow={() => seekForEditing(t.interactionFrom+12)} onRegenerateInteraction={regenerateInteraction} />
                    <label className="field-label" htmlFor="caption">最终照片描述 · English</label>
                    <textarea id="caption" className="input caption-input" maxLength={220} value={project.caption} placeholder="AI 根据照片生成并审校" onChange={e => update({ ...project, caption: e.target.value, captionReviewRequired: true, captionAudio: undefined, captionAudioSeconds: undefined, socialCopy: undefined })} />
                    <label className="field-label" htmlFor="captionChinese">最终照片描述 · 中文</label>
                    <textarea id="captionChinese" className="input caption-input" maxLength={220} value={project.captionChinese} placeholder="AI 根据画面生成自然中文" onChange={e => update({ ...project, captionChinese: e.target.value, captionReviewRequired: true, socialCopy: undefined })} />
                    {project.captionReviewRequired && <p className="hint">当前描述尚未通过看图审校。手动输入后可审校当前内容，也可以重新生成。</p>}
                    <Button type="button" variant="secondary" size="sm" className="w-full mt-2" disabled={!project.image || !project.caption.trim()} onClick={reviewCurrentCaption}><Sparkles />审校当前描述</Button>
            ...
            {mode === 'publish' ? <aside className="panel settings publish-settings"><div className="panel-heading"><h2>发布助手</h2><Sparkles size={17} /></div><fieldset className="panel-body" disabled={!!busy || !ready}>
                ...
                <Button variant="secondary" className="w-full mt-4" disabled={project.captionReviewRequired || !project.caption.trim() || !project.words.length || project.words.some(w => !w.english.trim())} onClick={speech}><Volume2 />生成全部配音</Button><p className="hint">会生成每个单词和已审校照片描述及已开启互动句的配音。修改描述后需重新生成并审校。</p><Button variant="outline" className="w-full mt-2" disabled={!project.image} onClick={() => { player.current?.seekTo(0); player.current?.play(); }}><Play />从头预览</Button>
            </fieldset></aside>}
        </main>
