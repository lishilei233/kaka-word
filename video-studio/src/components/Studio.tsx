...
export function Studio() {
    ...
    async function importFile(file?: File) {
        ...
        await run('导入素材', async () => {
            if (file.type.startsWith('video/')) {
                ...
                update({ ...project, image: undefined, video: undefined, words: [], interaction: undefined, sceneTheme: undefined, caption: '', captionChinese: '', captionSentences: undefined, captionReviewRequired: true, captionAudio: undefined, captionAudioSeconds: undefined }); setSelected('');
                ...
                if (imageExt === 'heic' || imageExt === 'heif') {
                    ...
                    try {
                        ...
                        update({ ...project, ...frame, video: undefined, words: [], interaction: undefined, sceneTheme: undefined, caption: '', captionChinese: '', captionSentences: undefined, captionReviewRequired: true, captionAudio: undefined, captionAudioSeconds: undefined }); setSelected('');
    ...
    async function importFiles(files: File[]) {
        ...
        await run('导入 Apple 动态照片', async () => {
            ...
            setSource(videoUrl);
            update({ ...project, ...frame, video: videoUrl, captureSeconds: 0, introSeconds: 2, words: [], interaction: undefined, sceneTheme: undefined, caption: '', captionChinese: '', captionSentences: undefined, captionReviewRequired: true, captionAudio: undefined, captionAudioSeconds: undefined });
    ...
    async function capture() {
        ...
        await run('截取照片', async () => {
            ...
            update({ ...project, ...frame, video: source, captureSeconds: element.currentTime, words: [], interaction: undefined, sceneTheme: undefined, caption: '', captionChinese: '', captionSentences: undefined, captionReviewRequired: true, captionAudio: undefined, captionAudioSeconds: undefined });
        ...
        <main className="workspace">
            <aside className="panel materials"><div className="panel-heading"><span className="section-number">01</span><h2>素材与单词</h2><ImagePlus size={17} /></div>
                <fieldset disabled={!!busy || !!job || !ready} className="panel-body">
                    ...
                    {project.captionSentences ? project.captionSentences.map((sentence, index) => <div key={index}>
                        <label className="field-label" htmlFor={`caption-${index}`}>照片描述 · 第 {index + 1} 句 · English</label>
                        <textarea id={`caption-${index}`} className="input caption-input" maxLength={220} value={sentence.english} onChange={e => update(editCaptionSentence(project, index, { english: e.target.value }))} />
                        <label className="field-label" htmlFor={`captionChinese-${index}`}>第 {index + 1} 句 · 中文</label>
